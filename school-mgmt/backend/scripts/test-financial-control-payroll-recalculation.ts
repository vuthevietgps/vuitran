...
async function testSalaryConfigAndPayrollVersioning() {
  ...
  // net draft = 7,500,000 - late(2*50,000) = 7,400,000
  assert.equal(payroll.baseSalaryAmount, 7_500_000, 'Salary config + attendance ratio must drive base salary amount');
  assert.equal(payroll.netAmount, 7_400_000, 'Staff payroll draft net must follow configured salary formula');
  assert.equal(payroll.status, 'DRAFT', 'Generated payroll must start as DRAFT');

  // Update payroll version (manual adjustment in DRAFT) before approval.
  const updatedDraft = await service.update(String(payroll._id), {
    bonusAmount: 200_000,
    deductionAmount: 50_000,
  });
  assert.equal(updatedDraft.netAmount, 7_550_000, 'Payroll draft update must recalculate net amount');
...
async function main() {
  const setup = await testSalaryConfigAndPayrollVersioning();
  await testPayrollImpactToFinancialControl(setup);
  console.log('PASS: payroll recalculation -> financial-control regression checks');
}

main().catch((err) => {
  console.error('FAIL: payroll recalculation -> financial-control regression checks');
  console.error(err);
  process.exit(1);
});
