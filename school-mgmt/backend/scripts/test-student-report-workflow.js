...
async function main() {
  console.log(`Testing student-report workflow against ${API_BASE}`);
  ...
  console.log('- DIRECTOR, TEACHER, ACCOUNTING logged in');

  const teacherId = normalizeId(teacher.user);
  ensure(teacherId, 'Teacher demo id missing');

  const suffix = uniqueSuffix();
  const studentCode = `SRPT-${suffix}`;
  const studentName = `Student Report ${suffix}`;
  const classCode = `CLS-SR-${suffix}`;
  const className = `Class Student Report ${suffix}`;
  ...
  await step('Frontend route and RBAC baseline are correct', async () => {
    const routeStatus = await fetchRoute(`${FRONTEND_BASE}/app/student-report`);
    ensure(routeStatus === 200, `Expected /app/student-report => 200, got ${routeStatus}`);

    await request({
      reqPath: '/students/report',
      token: teacher.token,
      expectedStatus: [403],
    });
  });

  await step('Director can create student and class, teacher can mark attendance', async () => {
    const createdStudent = await request({
      ...
      body: {
        ...
        parentPhone: `090${suffix.slice(-7)}`,
    ...
    await request({
      ...
      body: {
        ...
        date: attendanceDate,
        status: 'PRESENT',
        notes: 'student-report-workflow',
      },
    });
  });

  await step('Student report returns the new student with attendance count', async () => {
    const report = await request({
      reqPath: `/students/report?searchTerm=${encodeURIComponent(studentName)}`,
      token: director.token,
      expectedStatus: [200],
    });
  ...
  await step('Comprehensive report also reflects the created class and attendance', async () => {
    const comprehensive = await request({
      reqPath: `/students/comprehensive-report?classId=${classId}`,
      token: director.token,
      expectedStatus: [200],
    });
    ensure(comprehensive.data && Array.isArray(comprehensive.data.rows), 'Comprehensive report missing rows');
    ensure(comprehensive.data.rows.length >= 1, 'Comprehensive report returned no rows');
    const row = comprehensive.data.rows.find(
      (item) => item.studentCode === studentCode || normalizeId(item.studentId) === studentId,
    );
    ensure(row, 'Created student not found in comprehensive report');
    ensure(Array.isArray(row.sessions), 'Comprehensive report row missing sessions array');
    ensure(row.classCode === classCode, 'Comprehensive report classCode mismatch');
  });

  await step('Teacher remains blocked from report endpoints while accounting can read them', async () => {
    await request({
    ...
    await request({
      reqPath: `/students/report?searchTerm=${encodeURIComponent(studentCode)}`,
      token: accounting.token,
      expectedStatus: [200],
    });
  });

  console.log('\n=== Student Report Workflow Summary ===');
