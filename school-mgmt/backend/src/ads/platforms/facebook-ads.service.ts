  ...
  implements IAdPlatformService
{
  ...
  async syncBusinessToken(
    ...
    return this.runFacebookBusinessTokenSync(tokenDoc, accessToken, syncDates);
  }
  ...
  async syncPlatformCosts(
    ...
    return this.syncFacebookCosts(accessToken, account, groups, syncDate);
  }
  ...
  async syncFacebookBusinessToken(
    ...
    if (
      tokenDoc.platform !== "FACEBOOK" ||
      tokenDoc.tokenType !== ApiTokenType.FACEBOOK_SYSTEM_USER
    ) {
      throw new BadRequestException(
        "Token này không phải Facebook system user token.",
      );
    }
    ...
    const result = await this.runFacebookBusinessTokenSync(
      tokenDoc,
      accessToken,
      syncDates,
    );
  ...
  private async fetchFacebookCollection<T>(
    url: string,
    accessToken: string,
  ): Promise<T[]> {
    const results: T[] = [];
  ...
  private mapFacebookAdGroupStatus(raw?: string): string {
    const normalized = String(raw || "")
      .trim()
      .toUpperCase();
    if (!normalized || normalized.includes("ACTIVE"))
      return AdGroupStatus.ACTIVE;
  ...
  private async resolveFacebookBusinesses(
    ...
    try {
      const discovered =
        await this.fetchFacebookCollection<FacebookBusinessRef>(
      ...
      if (!businesses.size) {
        this.logger.warn(
          `Cannot resolve Facebook businesses from system token: ${err.message}`,
        );
      }
    }
  ...
  private async fetchFacebookBusinessAdAccounts(
    ...
    const addAccount = (
      ...
      business?: FacebookBusinessRef,
    ) => {
      const accountId = this.normalizeFacebookAdAccountId(
    ...
    for (const business of businesses) {
      for (const edge of ["owned_ad_accounts", "client_ad_accounts"]) {
        try {
          ...
          this.logger.warn(
            `Cannot fetch ${edge} for business ${business.id}: ${err.message}`,
          );
        }
      }
    }
    ...
    if (!accounts.size) {
      const fallbackRows =
        await this.fetchFacebookCollection<FacebookAdAccountRow>(
  ...
  private async fetchFacebookPages(
    ...
    for (const business of businesses) {
      try {
        ...
        this.logger.warn(
          `Cannot fetch owned_pages for business ${business.id}: ${err.message}`,
        );
      }
    }
    ...
    try {
      const accessiblePages =
        await this.fetchFacebookCollection<FacebookPageRow>(
      ...
      if (!pages.size) {
        this.logger.warn(
          `Cannot fetch /me/accounts for Facebook pages: ${err.message}`,
        );
      }
    }
    ...
    for (const page of pages.values()) {
      if (!page.access_token && page.id) {
        page.access_token = await this.resolveFacebookPageAccessToken(
          page.id,
          accessToken,
        );
      }
  ...
  private async upsertFacebookAdAccount(
    ...
    const platformAccountId = this.normalizeFacebookAdAccountId(
      row.account_id || row.id,
    );
    ...
    const businessName =
      row.business?.name || tokenDoc.businessName || undefined;
    const now = new Date();
  ...
  private async upsertFacebookFanpage(
    ...
    const now = new Date();
    const encryptedPageToken = row.access_token
      ? this.encrypt(row.access_token)
      : undefined;
    let fanpage = await this.fanpageModel
      .findOne({
  ...
  private async syncFacebookInsightsForAccount(
    ...
    for (const row of rows) {
      ...
      let group: AdGroupDocument | null | undefined =
        groupsByExternalId.get(externalId);
      if (!group) {
        group = await this.upsertFacebookAdGroup(
          account,
          {
            id: externalId,
            name: row.adset_name,
  ...
  private async runFacebookBusinessTokenSync(
    ...
    const businesses = await this.resolveFacebookBusinesses(
      tokenDoc,
      accessToken,
    );
    ...
    const accountRows = await this.fetchFacebookBusinessAdAccounts(
      accessToken,
      businesses,
    );
    ...
    for (const row of accountRows) {
      try {
        ...
        errors.push(
          `Ad account ${row.name || row.account_id || row.id}: ${err.message}`,
        );
      }
    }
    ...
    for (const row of pages) {
      try {
        ...
        if (!row.access_token) {
          errors.push(
            `Fanpage ${row.name || row.id}: chưa lấy được page access token.`,
          );
        }
      } catch (err: any) {
    ...
    for (const account of accounts) {
      try {
        const groupResult = await this.syncFacebookAdsetsForAccount(
          accessToken,
          account,
          tokenDoc,
        );
  ...
  private async syncFacebookCosts(
    ...
    try {
      const url =
        `https://graph.facebook.com/v25.0/act_${account.platformAccountId}/insights` +
        `?fields=adset_id,spend,impressions,clicks,actions` +
        `&level=adset` +
