...
const TEST_UPLOAD_FILE = path.join(
  ROOT_DIR,
  'runtime-logs',
  'ui-smoke-1775188772678',
...
async function openVideoContextForRole(page: Page, request: Parameters<typeof loginAsRole>[0], role: 'sale' | 'director', targetPath: string): Promise<void> {
  await page.goto(new URL('/login', APP_BASE_URL).toString());
  await page.evaluate(() => {
    localStorage.clear();
...
test('record menus and linked views after order approval', async ({ browser, request }) => {
  test.slow();
  test.setTimeout(300_000);

  const label = `LINKED-${Date.now()}`;
  ...
  const studentName = seededOrder.studentName as string;
  const invoiceNumber = seededOrder.items?.[0]?.invoiceNumber as string;
  ...
  try {
    await page.setContent(`
      ...
      <div class="card">
        <h1>Order Approval Linked Views</h1>
        <p>Demo nay show menu theo role, phe duyet order, va kiem tra cac man lien doi sau khi duyet.</p>
        <ul>
          <li>Sale sidebar khong con menu classes va agents.</li>
          <li>Director sidebar van thay classes va agents.</li>
          <li>Sau khi phe duyet, detail order, invoices va students cung phan anh du lieu moi.</li>
        </ul>
      </div>
    `);
    await page.waitForTimeout(1800);

    await setVideoLabel(page, 'Buoc 1: Mo menu voi role Sale. Ky vong sidebar khong hien link /app/classes va /app/agents.');
    await openVideoContextForRole(page, request, 'sale', '/app/orders');
    await expect(page.locator('aside.sidebar')).toBeVisible({ timeout: 20_000 });
    await expect(page.locator('aside.sidebar a[href="/app/classes"]')).toHaveCount(0);
    await expect(page.locator('aside.sidebar a[href="/app/agents"]')).toHaveCount(0);
    await page.waitForTimeout(1800);

    await setVideoLabel(page, 'Buoc 2: Chuyen sang role Director. Ky vong sidebar hien day du classes va agents de vao cac man lien doi.');
    await openVideoContextForRole(page, request, 'director', '/app/orders');
    await expect(page.locator('aside.sidebar a[href="/app/classes"]')).toBeVisible({ timeout: 20_000 });
    await expect(page.locator('aside.sidebar a[href="/app/agents"]')).toBeVisible({ timeout: 20_000 });
    await page.waitForTimeout(1800);

    await setVideoLabel(page, 'Buoc 3: Director loc order da submit san, mo chi tiet va phe duyet order.');
    ...
    await expect(page.getByTestId('order-approve-modal')).toBeHidden({ timeout: 20_000 });
    await expect(page.getByTestId('order-detail-modal').locator('.badge').first()).toContainText(/Da duyet|APPROVED|Đã duyệt/i, { timeout: 20_000 });
    await expect(page.getByTestId('order-detail-modal').getByText(/Invoice:\s*1/i)).toBeVisible({ timeout: 20_000 });
    await page.waitForTimeout(1500);

    await clickLocator(page, page.getByTestId('order-detail-close'));
    await expect(page.getByTestId('order-detail-modal')).toBeHidden({ timeout: 15_000 });

    await setVideoLabel(page, 'Buoc 4: Dong chi tiet order, roi di sang Invoices bang menu trai. Ky vong invoice moi xuat hien voi status da duyet.');
    await clickLocator(page, page.locator('aside.sidebar a[href="/app/invoices"]').first());
    await expect(page).toHaveURL(/\/app\/invoices/, { timeout: 20_000 });
    const invoiceKeyword = page.locator('section.filters-section input').first();
    await slowFill(page, invoiceKeyword, invoiceNumber);
    const invoiceRow = page.getByTestId('invoice-row').filter({ hasText: invoiceNumber }).first();
    await expect(invoiceRow).toBeVisible({ timeout: 20_000 });
    await expect(invoiceRow).toContainText(studentName, { timeout: 20_000 });
    await expect(invoiceRow).toContainText(/Da duyet|APPROVED|Đã duyệt/i, { timeout: 20_000 });
    await page.waitForTimeout(1800);

    await setVideoLabel(page, 'Buoc 5: Sang Students bang menu trai. Ky vong hoc sinh vua provision sau duyet order da hien tren danh sach.');
    await clickLocator(page, page.locator('aside.sidebar a[href="/app/students"]').first());
    await expect(page).toHaveURL(/\/app\/students/, { timeout: 20_000 });
    const studentKeyword = page.locator('section.filters input').first();
    await slowFill(page, studentKeyword, studentName);
    const studentRow = page.locator('table.data tbody tr').filter({ hasText: studentName }).first();
    await expect(studentRow).toBeVisible({ timeout: 20_000 });
    await expect(studentRow).toContainText(studentName, { timeout: 20_000 });
    await page.waitForTimeout(1800);

    await setVideoLabel(page, 'Ket qua: menu theo role da dung va sau khi duyet order, cac man lien doi Orders, Invoices, Students deu phan anh du lieu moi.');
    await page.waitForTimeout(1800);
    await setVideoLabel(page, null);
  } finally {
    await saveRecordedVideo(page, context, `UI_Order_Approval_Linked_Views_${DATE_STAMP}`);
  }
});
