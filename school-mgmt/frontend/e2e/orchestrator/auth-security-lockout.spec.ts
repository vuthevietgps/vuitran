  ...
  try {
    ...
    for (let attempt = 1; attempt <= 5; attempt += 1) {
      ...
      if (result.status === 429) {
        ...
        expect(result.status, `Unexpected status on failed attempt ${attempt}`).toBe(401);
    ...
    await expect(batch.page).toHaveURL(/\/login$/);
    await expect(batch.page.getByTestId('login-form')).toBeVisible();
    expect(lockedResult.status, 'Locked account should not authenticate successfully').not.toBe(200);

    const uiErrorText = (await loginError.textContent()) || '';
    const evidenceText = `${uiErrorText} ${lockedResult.bodyText}`.toLowerCase();
    expect(
      hasLockoutKeyword(evidenceText),
      `Expected lockout wording in UI or backend response, got: ${evidenceText}`,
    ).toBe(true);

    if (lockedResult.status === 429) {
      await expect(loginError).toHaveText('Ban da thu qua nhieu lan. Vui long doi 1 phut.');
    } else {
      expect(lockedResult.status, 'Sixth attempt with the real password should be rejected while locked').toBe(401);
      await expect(loginError).toContainText(/khoa|khóa|thu lai sau|thử lại sau|qua nhieu lan|quá nhiều lần|lock/i);
    }
    ...
    await batch.finalize('PASS', {
      extraLines: [
        ...
        `Last response body: ${lastBodyText}`,
