...
import { SHAREHOLDER_VIDEO_COPY } from './shareholder-video-copy';
...
const FINAL_VIDEO_BASENAME = `UI_CoDong_Full_Workflow_VI_${DATE_STAMP}`;
const VIEWPORT = { width: 1600, height: 900 };
const VIDEO_TEST_TITLE = 'records the full shareholder master workflow with HD demo pacing';
const DIRECTOR_VIDEO_COPY: any = SHAREHOLDER_VIDEO_COPY;
...
async function writeNarrationArtifacts(artifactDir: string, cues: NarrationCue[]): Promise<void> {
  ...
  const lines = [
    '# Shareholder Full Workflow Narration',
    '',
    'Danh sach cue de render long tieng cho video co dong.',
    '',
...
async function showTitleCard(page: Page, title: string, subtitle: string, bullets: string[]): Promise<void> {
  await page.setContent(`
    ...
    <div class="card">
      <div class="eyebrow">Co Dong Workflow</div>
      <h1>${title}</h1>
      <p>${subtitle}</p>
...
async function showWorkflowRoadmapCard(
  ...
  await page.setContent(`
    ...
    <div class="shell">
      <div class="eyebrow">Co Dong Roadmap</div>
      <h1>${title}</h1>
...
function collectNarrationTexts(copy: typeof DIRECTOR_VIDEO_COPY): string[] {
  return [
    ...
    copy.agingSceneNarration,
    copy.agingNarration,
    copy.loansSceneNarration,
    copy.loansNarration,
    copy.adsSceneNarration,
    copy.adsNarration,
    copy.teachingSceneNarration,
    copy.teachingNarration,
    copy.comprehensiveSceneNarration,
    copy.comprehensiveNarration,
...
type ShareholderWorkflowState = {
  ...
  classes: any[];
  sessions: any[];
  comprehensiveRows: any[];
};

function buildShareholderInvestorMetrics(monthCount: number, loanSummary: any): any {
  const normalized = [3, 6, 12].includes(monthCount) ? monthCount : 6;
  ...
  const revenueMap: Record<number, number[]> = {
    3: [4_200_000, 4_750_000, 5_400_000],
    6: [3_400_000, 3_900_000, 4_250_000, 4_700_000, 5_100_000, 5_400_000],
    12: [2_400_000, 2_650_000, 2_900_000, 3_200_000, 3_550_000, 3_900_000, 4_250_000, 4_500_000, 4_780_000, 5_050_000, 5_220_000, 5_400_000],
  };
  const activeStudentsMap: Record<number, number[]> = {
    3: [42, 47, 51],
    6: [31, 35, 39, 43, 47, 51],
    12: [18, 20, 23, 26, 29, 31, 35, 39, 42, 45, 48, 51],
  };

  const activeStudents = activeStudentsMap[normalized][activeStudentsMap[normalized].length - 1];
  const enrolledStudents = activeStudents + 7;
  const revenues = revenueMap[normalized];

  return {
    snapshot: {
      cashOnHand: 12_600_000,
      totalFundBalance: 4_100_000,
      burnRate: 1_450_000,
      runway: 8.7,
    },
    revenue: {
      recognizedRevenue: {
        thisMonth: revenues[revenues.length - 1],
        lastMonth: revenues[revenues.length - 2] || revenues[revenues.length - 1],
        ytd: 53_800_000,
        growthPercent: 12.8,
      },
      unearnedRevenue: {
        walletBalance: 2_250_000,
        unconsumedInvoiceValue: 4_900_000,
        total: 7_150_000,
      },
    },
    profitability: {
      grossProfit: 21_600_000,
      grossMargin: 43.1,
      netProfit: 10_850_000,
      netMargin: 21.4,
      ebitda: 12_100_000,
      ebitdaMargin: 23.9,
    },
    unitEconomics: {
      cac: 720_000,
      costPerLead: 118_000,
      costPerOrder: 462_000,
      ltv: 5_800_000,
      ltvCacRatio: 8.1,
      arpu: 1_960_000,
      arpuActive: 2_140_000,
      activeStudents,
      enrolledStudents,
    },
    customerBase: {
    ...
    methodology: {
      cac: { formula: 'Ads spend / don moi', note: 'Lay tu dashboard ads va order da duyet.' },
      ...
      arpu: {
        ...
        note: 'Tach rieng enrolled va active.',
      },
    },
    retention: {
      retentionRate: 84.2,
    ...
    liabilities: {
      accountsReceivable: 3_250_000,
      payrollPayable: 2_450_000,
      expensePayable: 1_250_000,
      loanSummary: { totalDebt: loanSummary.totalDebt },
      arAgingBuckets: { current: 400_000, overdue30: 600_000, overdue60: 1_200_000, overdue90: 1_050_000 },
    },
    trend: {
      monthCount: normalized,
      months: monthsMap[normalized],
      revenue: revenues,
      netProfit: revenues.map((value, index) => Math.round(value * 0.21) + (index * 10_000)),
      studentCount: activeStudentsMap[normalized],
    },
  };
}

function buildShareholderDashboard(state: ShareholderWorkflowState): any {
  const bankBalance = state.financial.bankAccounts.reduce((sum, item) => sum + Number(item.currentBalance || 0), 0);
  const fundBalance = state.financial.funds.reduce((sum, item) => sum + Number(item.currentBalance || 0), 0);
  return {
    cashPosition: {
      bankBalance,
      bankAccountCount: state.financial.bankAccounts.length,
      fundBalance,
      fundCount: state.financial.funds.length,
      marketingFund: 2_100_000,
      marketingFundCount: 1,
      availableCash: bankBalance + fundBalance,
    },
    obligations: {
      payrollPayable: 2_450_000,
      payrollPayableCount: 5,
      expensePayable: 1_250_000,
      expensePayableCount: 4,
      orderPayable: 550_000,
      orderPayableCount: 2,
      totalPayable14Days: 4_250_000,
      operatingReserve3Months: 4_350_000,
      burnRate: 1_450_000,
      runway: 8.7,
      reserveHealthy: true,
      cashAfterObligations: 8_350_000,
    },
    deferredRevenue: {
      walletBalance: 2_250_000,
      walletCount: 4,
      pendingInvoiceAmount: 4_900_000,
    ...
    metrics: {
      ...
      netProfit: 10_850_000,
      revenueGrowth: 12.8,
      thisMonthRevenue: 5_400_000,
      lastMonthRevenue: 5_100_000,
      accountsReceivable: 3_250_000,
      accountsPayable: 4_250_000,
      deferredRevenue: 7_150_000,
    },
    debtPosition: {
      totalDebt: state.loans.summary.totalDebt,
      activeLoanCount: state.loans.summary.activeLoanCount,
      loanPayable: state.loans.summary.upcomingPayments30d,
      loanPayableCount: state.loans.summary.upcomingPaymentCount30d,
    },
    fundWarnings: [
      {
        fundId: 'fund-shareholder-marketing',
        fundName: 'Quy Marketing',
        severity: 'WARNING',
        message: 'So du quy marketing con thap hon muc target da dat.',
      },
    ],
  };
}

function buildShareholderAlerts(): any {
  return {
    totalAlerts: 3,
    criticalCount: 1,
    warningCount: 2,
    infoCount: 0,
    marketingBudget: {
      fundBalance: 2_100_000,
      optimalDailyBudget: 180_000,
      optimalMonthlyBudget: 5_400_000,
      groupBreakdown: [],
    },
    alerts: [
      {
        id: 'shareholder-alert-001',
        severity: 'CRITICAL',
        category: 'RECONCILIATION',
        title: 'Con muc chua doi soat',
        message: 'Van con giao dich ngan hang chua doi soat cuoi ky.',
        data: { unreconciledCount: 1 },
        actions: [],
      },
      {
        id: 'shareholder-alert-002',
        severity: 'WARNING',
        category: 'FUND',
        title: 'Quy marketing duoi muc muc tieu',
        message: 'Can theo doi toc do nap quy so voi ke hoach campaign.',
        data: { targetBalance: 4_000_000 },
        actions: [],
      },
      {
        id: 'shareholder-alert-003',
        severity: 'WARNING',
        category: 'LOAN',
        title: 'Co ky tra no qua han',
        message: 'Mot ky thanh toan nha dau tu noi bo da qua han.',
        data: { overdueAmount: 450_000 },
        actions: [],
      },
    ],
  };
...
function buildShareholderWorkflowState(): ShareholderWorkflowState {
  const analytics = createAnalyticsState();

  return {
    sessionActive: false,
    user: {
      _id: 'shareholder-demo-user-001',
      sub: 'shareholder-demo-user-001',
    ...
    financial: {
      bankAccounts: [
        {
          _id: 'bank-account-shareholder-001',
          accountCode: 'BA-CD-001',
          bankName: 'Vietcombank',
          accountNumber: '001122334455',
          accountHolder: 'School Operating',
          currentBalance: 8_500_000,
          openingBalance: 7_900_000,
          status: 'ACTIVE',
          isPrimary: true,
          createdAt: '2026-04-01T00:00:00.000Z',
        },
      ...
      bankTransactions: [
        {
          _id: 'bank-tx-shareholder-001',
          transactionCode: 'BTX-CD-001',
          bankAccountId: 'bank-account-shareholder-001',
          type: 'DEPOSIT',
          category: 'TUITION_INCOME',
          amount: 1_250_000,
          balanceBefore: 7_250_000,
          balanceAfter: 8_500_000,
          transactionDate: '2026-04-09T00:00:00.000Z',
          description: 'Thu hoc phi dot 2',
          recordedByName: 'Finance Ops',
          isReconciled: false,
          createdAt: '2026-04-09T08:30:00.000Z',
        },
        {
          _id: 'bank-tx-shareholder-002',
          transactionCode: 'BTX-CD-002',
          bankAccountId: 'bank-account-shareholder-001',
          type: 'WITHDRAW',
          category: 'PAYROLL',
          amount: 920_000,
          balanceBefore: 8_170_000,
          balanceAfter: 7_250_000,
          transactionDate: '2026-04-07T00:00:00.000Z',
          description: 'Luong giao vien dot 1',
          recordedByName: 'Finance Ops',
          isReconciled: true,
      ...
      funds: [
        {
          _id: 'fund-shareholder-marketing',
          fundCode: 'FUND-MKT',
          name: 'Quy Marketing',
          fundType: 'MARKETING',
          ...
          totalWithdrawn: 2_900_000,
          createdAt: '2026-04-01T00:00:00.000Z',
        },
        {
          _id: 'fund-shareholder-reserve',
          fundCode: 'FUND-RSV',
          name: 'Quy Du phong',
          fundType: 'RESERVE',
          currentBalance: 2_000_000,
          minimumBalance: 1_800_000,
          targetBalance: 3_500_000,
          status: 'ACTIVE',
      ...
      fundTransactions: [
        {
          _id: 'fund-tx-shareholder-001',
          transactionCode: 'FTX-CD-001',
          fundId: 'fund-shareholder-marketing',
          type: 'WITHDRAW',
          amount: 500_000,
          balanceBefore: 2_600_000,
          balanceAfter: 2_100_000,
          transactionDate: '2026-04-08T00:00:00.000Z',
          description: 'Chi campaign search',
          performedByName: 'Ads Lead',
    ...
    agingReport: {
      ...
      details: [
        {
          parentName: 'PH Demo 1',
          parentPhone: '0900000001',
          students: ['Hoc sinh A'],
          totalDebt: 1_200_000,
          oldestDate: '2026-02-15T00:00:00.000Z',
          bucket: '31-60',
          items: [],
        },
        {
          parentName: 'PH Demo 2',
          parentPhone: '0900000002',
          students: ['Hoc sinh B', 'Hoc sinh C'],
          totalDebt: 700_000,
          oldestDate: '2026-01-12T00:00:00.000Z',
          bucket: '90+',
          items: [],
        },
        {
          parentName: 'PH Demo 3',
          parentPhone: '0900000003',
          students: ['Hoc sinh D'],
          totalDebt: 600_000,
          oldestDate: '2026-03-20T00:00:00.000Z',
          bucket: '1-30',
          items: [],
        },
      ],
    ...
    loans: {
      summary: {
        ...
        overdueCount: 1,
        upcomingPayments30d: 1_200_000,
        upcomingPaymentCount30d: 1,
        totalInterestPaid: 920_000,
        totalAmountPaid: 3_100_000,
      },
      loans: [
        {
          _id: 'loan-shareholder-001',
          loanCode: 'LOAN-001',
          ...
          collateral: 'Hop dong doanh thu',
          notes: 'Khoan vay bo sung von luu dong',
          createdByName: 'Finance Director',
          approvedByName: 'Board',
          createdAt: '2026-01-05T00:00:00.000Z',
        },
        {
          _id: 'loan-shareholder-002',
          loanCode: 'LOAN-002',
          lenderName: 'Nha dau tu noi bo',
          ...
          collateral: '',
          notes: 'Khoan vay mo rong trung tam',
          createdByName: 'Finance Director',
          approvedByName: 'Board',
      ...
      payments: [
        {
          _id: 'loan-payment-shareholder-001',
          paymentCode: 'LPAY-001',
          loanId: 'loan-shareholder-001',
          paymentNumber: 1,
          dueDate: '2026-03-05T00:00:00.000Z',
          paidDate: '2026-03-05T00:00:00.000Z',
          principalAmount: 900_000,
          interestAmount: 140_000,
          totalAmount: 1_040_000,
          status: 'PAID',
          paymentMethod: 'BANK_TRANSFER',
          paidByName: 'Finance Ops',
          createdAt: '2026-03-05T00:00:00.000Z',
        },
        {
          _id: 'loan-payment-shareholder-002',
          paymentCode: 'LPAY-002',
          loanId: 'loan-shareholder-001',
          paymentNumber: 2,
          dueDate: '2026-05-05T00:00:00.000Z',
          principalAmount: 930_000,
        ...
        {
          _id: 'loan-payment-shareholder-003',
          paymentCode: 'LPAY-003',
          loanId: 'loan-shareholder-002',
          paymentNumber: 1,
    ...
    classes: [
      {
        _id: 'class-shareholder-001',
        code: 'TA101',
        name: 'Lop Tieng Anh Nang cao 1',
        classMode: 'OFFLINE',
      },
      {
        _id: 'class-shareholder-002',
        code: 'TO201',
        name: 'Lop Toan Tu duy 2',
        classMode: 'ONLINE',
      },
    ],
    sessions: [
      {
        _id: 'session-shareholder-001',
        classId: { _id: 'class-shareholder-001', name: 'Lop Tieng Anh Nang cao 1', code: 'TA101', classMode: 'OFFLINE' },
        studentId: { _id: 'student-shareholder-001', fullName: 'Nguyen Hoang An', studentCode: 'HS001' },
        teacherId: { _id: 'teacher-shareholder-001', fullName: 'Co Lan' },
        ...
        teachingReport: {
          lessonContent: 'On tap cau truc thi hien tai don va luyen nghe chu de school life.',
          studentAttitude: 'Hoc sinh tap trung, chu dong tra loi va hoan thanh bai tap tren lop.',
          teacherComment: 'Can duy tri toc do doc hieu va tang tu vung hoc thuat.',
          homework: 'Lam worksheet trang 12-14 va ghi am doan hoi thoai.',
          additionalNotes: 'Phu huynh da duoc cap nhat tien do hoc tap sau buoi hoc.',
          submittedAt: '2026-04-08T20:10:00.000Z',
        },
        confirmation: {
          teacherCompletedAt: '2026-04-08T19:40:00.000Z',
          finalizedAt: '2026-04-08T20:15:00.000Z',
        },
      },
      {
        _id: 'session-shareholder-002',
        classId: { _id: 'class-shareholder-002', name: 'Lop Toan Tu duy 2', code: 'TO201', classMode: 'ONLINE' },
        studentId: { _id: 'student-shareholder-002', fullName: 'Pham Minh Khoa', studentCode: 'HS002' },
        teacherId: { _id: 'teacher-shareholder-002', fullName: 'Thay Binh' },
        scheduledDate: '2026-04-09T00:00:00.000Z',
        scheduledStartTime: '19:00',
        scheduledEndTime: '20:30',
        durationMinutes: 90,
        amountCharged: 420_000,
        teacherPayout: 210_000,
        status: 'COMPLETED',
        isPaid: true,
        isTeacherPaid: false,
        hasTeachingReport: false,
        confirmation: {
          teacherCompletedAt: '2026-04-09T20:35:00.000Z',
        },
      },
    ],
    comprehensiveRows: [
      {
        studentId: 'student-shareholder-001',
        studentCode: 'HS001',
        fullName: 'Nguyen Hoang An',
        age: 10,
        parentName: 'PH Demo 1',
        parentPhone: '0900000001',
        faceImage: '',
        classMode: 'OFFLINE',
        classId: 'class-shareholder-001',
        classCode: 'TA101',
        className: 'Lop Tieng Anh Nang cao 1',
        subject: 'Tieng Anh',
        grade: 'Grade 4',
        level: 'A2',
        ...
        teacherName: 'Co Lan',
        teacherCode: 'GV001',
        teacherCodeAndName: 'GV001 - Co Lan',
        teacherSalary: 220_000,
        teacherSalaryType: 'PER_SESSION',
        invoiceNumber: 'INV-TA101-01',
        saleId: 'sale-001',
        saleName: 'Sale An',
        dataStatus: 'ACTIVE',
        ...
        absentCount: 0,
        sessions: [
          { date: '2026-04-01', status: 'PRESENT', attendedAt: '2026-04-01T18:01:00.000Z', duration: 90, teacherCode: 'GV001', teacherName: 'Co Lan' },
          { date: '2026-04-03', status: 'PRESENT', attendedAt: '2026-04-03T18:02:00.000Z', duration: 90, teacherCode: 'GV001', teacherName: 'Co Lan' },
          { date: '2026-04-05', status: 'PRESENT', attendedAt: '2026-04-05T18:00:00.000Z', duration: 90, teacherCode: 'GV001', teacherName: 'Co Lan' },
          { date: '2026-04-08', status: 'PRESENT', attendedAt: '2026-04-08T18:00:00.000Z', duration: 90, teacherCode: 'GV001', teacherName: 'Co Lan' },
        ],
      },
      {
        studentId: 'student-shareholder-002',
        studentCode: 'HS002',
        ...
        faceImage: '',
        classMode: 'ONLINE',
        classId: 'class-shareholder-002',
        classCode: 'TO201',
        className: 'Lop Toan Tu duy 2',
        subject: 'Toan',
        grade: 'Grade 5',
        level: 'Logic 2',
        dateOfBirth: '2014-07-03',
        teacherName: 'Thay Binh',
        teacherCode: 'GV002',
        teacherCodeAndName: 'GV002 - Thay Binh',
        teacherSalary: 210_000,
        teacherSalaryType: 'PER_SESSION',
        invoiceNumber: 'INV-TO201-02',
        saleId: 'sale-002',
        saleName: 'Sale Binh',
        dataStatus: 'TRIAL',
        ...
        absentCount: 1,
        sessions: [
          { date: '2026-04-02', status: 'PRESENT', attendedAt: '2026-04-02T19:03:00.000Z', duration: 90, teacherCode: 'GV002', teacherName: 'Thay Binh' },
          { date: '2026-04-06', status: 'ABSENT', attendedAt: null, duration: 0, teacherCode: 'GV002', teacherName: 'Thay Binh' },
          { date: '2026-04-09', status: 'PRESENT', attendedAt: '2026-04-09T19:01:00.000Z', duration: 90, teacherCode: 'GV002', teacherName: 'Thay Binh' },
          { date: null, status: null, attendedAt: null, duration: 0, teacherCode: '', teacherName: '' },
        ],
      },
    ],
    analytics,
...
function filterShareholderLoans(state: ShareholderWorkflowState, url: URL): any[] {
  const keyword = (url.searchParams.get('keyword') || '').trim().toLowerCase();
  const status = (url.searchParams.get('status') || '').trim();
  const lenderType = (url.searchParams.get('lenderType') || '').trim();

  return state.loans.loans.filter((loan) => {
    if (status && loan.status !== status) return false;
    if (lenderType && loan.lenderType !== lenderType) return false;
    if (keyword) {
      const haystack = `${loan.loanCode} ${loan.lenderName}`.toLowerCase();
...
function filterShareholderPayments(state: ShareholderWorkflowState, url: URL): any[] {
  const loanId = url.searchParams.get('loanId');
  const status = url.searchParams.get('status');
  const startDate = url.searchParams.get('startDate');
  const endDate = url.searchParams.get('endDate');

  return state.loans.payments.filter((payment) => {
    if (loanId && payment.loanId !== loanId) return false;
    if (status && payment.status !== status) return false;
    if (startDate && toDateOnly(payment.paidDate || payment.dueDate) < startDate) return false;
    if (endDate && toDateOnly(payment.paidDate || payment.dueDate) > endDate) return false;
    return true;
  });
}

function filterShareholderSessions(state: ShareholderWorkflowState, url: URL): any[] {
  const hasReport = url.searchParams.get('hasReport');
  const fromDate = url.searchParams.get('fromDate');
  const toDate = url.searchParams.get('toDate');

  return state.sessions.filter((session) => {
    const dateOnly = toDateOnly(session.scheduledDate);
    if (fromDate && dateOnly < fromDate) return false;
    if (toDate && dateOnly > toDate) return false;
    if (hasReport === 'true' && !session.hasTeachingReport) return false;
    if (hasReport === 'false' && session.hasTeachingReport) return false;
    return true;
  });
}

function filterShareholderComprehensiveRows(state: ShareholderWorkflowState, url: URL): any[] {
  const classId = url.searchParams.get('classId') || '';
  const searchTerm = (url.searchParams.get('searchTerm') || '').trim().toLowerCase();

  return state.comprehensiveRows.filter((row) => {
    if (classId && row.classId !== classId) return false;
    if (searchTerm) {
      const haystack = `${row.studentCode} ${row.fullName} ${row.parentName} ${row.parentPhone}`.toLowerCase();
...
class ShareholderWorkflowPage {
  ...
  async loginAsShareholder(): Promise<void> {
    await this.goto('/login');
    await expect(this.page.getByTestId('login-form')).toBeVisible();
    await this.slowFill(this.page.getByTestId('login-email'), 'shareholder.demo@school.local');
    await this.pause(500);
    await this.slowFill(this.page.getByTestId('login-password'), '123456');
    await this.pause();
    await clickLocator(this.page, this.page.getByTestId('login-submit'));
    await this.page.waitForURL(/\/app\/(dashboard|investor-dashboard)/, { timeout: 30_000 });
    if (!/\/app\/investor-dashboard/.test(this.page.url())) {
      await this.page.waitForURL(/\/app\/investor-dashboard/, { timeout: 30_000 });
    }
  }
}

async function installShareholderWorkflowRoutes(page: Page, state: ShareholderWorkflowState): Promise<void> {
  const API_MATCHER = new RegExp(
    `${API_PREFIX_PATTERN}/(?:auth|users|financial-control|loans|classes|students|sessions|ads|export|notifications)(?:/.*)?(?:\\?.*)?$`,
  );
  ...
  await page.route(API_MATCHER, async (route) => {
    ...
    const readLoginBody = () => {
      try {
        return request.postDataJSON() as { email?: string; password?: string };
      } catch {
        return {};
      }
    };
    ...
    if (pathname === '/auth/login') {
      const body = readLoginBody();
      state.sessionActive = true;
      state.loginBodies.push({
    ...
    if (pathname === '/users/me') {
      if (!state.sessionActive) {
        await jsonResponse(route, { message: 'Unauthorized' }, 401);
        return;
      }
      await jsonResponse(route, clone(state.user));
      return;
    ...
    if (pathname === '/notifications/unread-count') {
      await jsonResponse(route, { count: 0 });
      return;
    }

    if (pathname === '/financial-control/investor-metrics') {
      const monthCount = Number(url.searchParams.get('monthCount') || '6');
      await jsonResponse(route, buildShareholderInvestorMetrics(monthCount, state.loans.summary));
      return;
    }

    if (pathname === '/financial-control/dashboard') {
      await jsonResponse(route, buildShareholderDashboard(state));
      return;
    }

    if (pathname === '/financial-control/alerts') {
      await jsonResponse(route, buildShareholderAlerts());
      return;
    }

    if (pathname === '/financial-control/reconciliation') {
      await jsonResponse(route, reconciliationReport(state.financial));
      return;
    }

    if (pathname === '/financial-control/aging-report') {
      await jsonResponse(route, clone(state.agingReport));
      return;
    }

    if (pathname === '/financial-control/bank-accounts') {
      await jsonResponse(route, clone(state.financial.bankAccounts));
      return;
    }

    if (pathname === '/financial-control/bank-transactions') {
      await jsonResponse(route, clone(state.financial.bankTransactions));
      return;
    }

    if (pathname === '/financial-control/funds') {
      await jsonResponse(route, clone(state.financial.funds));
      return;
    }

    if (pathname === '/financial-control/fund-transactions') {
      const fundId = url.searchParams.get('fundId');
      const items = fundId
        ? state.financial.fundTransactions.filter((item) => item.fundId === fundId)
        : state.financial.fundTransactions;
      await jsonResponse(route, clone(items));
      return;
    }

    if (pathname === '/loans/summary') {
      await jsonResponse(route, clone(state.loans.summary));
      return;
    }

    if (pathname === '/loans/payments/list') {
      await jsonResponse(route, clone(filterShareholderPayments(state, url)));
      return;
    }

    if (pathname === '/loans') {
      await jsonResponse(route, clone(filterShareholderLoans(state, url)));
      return;
    }

    if (pathname.startsWith('/loans/')) {
      const loanId = pathname.split('/')[2] || '';
      const loan = state.loans.loans.find((item) => item._id === loanId);
      if (!loan) {
        await jsonResponse(route, { message: 'Not found' }, 404);
        return;
      }
      await jsonResponse(route, clone(loan));
      return;
    }
    ...
    if (pathname === '/students/comprehensive-report') {
      const rows = filterShareholderComprehensiveRows(state, url);
      await jsonResponse(route, {
        maxSessions: 4,
        rows: clone(rows),
      });
    ...
    if (pathname === '/sessions') {
      const sessions = filterShareholderSessions(state, url);
      await jsonResponse(route, {
        data: clone(sessions),
        meta: {
    ...
    if (pathname === '/ads/groups/all') {
      await jsonResponse(route, clone(state.analytics.groups));
      return;
    }

    if (pathname === '/ads/analytics/realized-cohort') {
      state.analytics.captured.realized.push(url);
      await jsonResponse(route, buildRealizedResponse(url, state.analytics));
      return;
    }

    if (pathname === '/ads/analytics/parents-profit') {
      state.analytics.captured.parents.push(url);
      await jsonResponse(route, buildParentProfitResponse(url, state.analytics));
      return;
    }

    if (pathname === '/ads/analytics') {
      state.analytics.captured.analytics.push(url);
      await jsonResponse(route, buildAnalyticsResponse(url, state.analytics));
      return;
    }

    if (pathname === '/export/investor-summary') {
      const csvBody = [
        '\uFEFF"Section","Metric","Value","Parent","Phone","Students","Bucket"',
        '"Aging","Tong cong no","3250000","PH #1","An danh","1 hoc sinh","31-60"',
      ].join('\n');
      ...
      await route.fulfill({
        ...
        headers: {
          ...
          'Content-Disposition': 'attachment; filename="bao-cao-co-dong.csv"',
        },
        body: csvBody,
      });
      return;
    ...
    await jsonResponse(route, { message: `Unhandled mock path: ${pathname}` }, 404);
  });
}

test.describe.serial('Shareholder full workflow video', () => {
  test(VIDEO_TEST_TITLE, async ({ page }) => {
    test.setTimeout(900_000);

    const state = buildShareholderWorkflowState();
    const ui = new ShareholderWorkflowPage(page);
    const context = page.context();
    const artifactDir = path.join(META_DIR, `shareholder-demo-${Date.now()}`);
    ...
    await installShareholderWorkflowRoutes(page, state);
    ...
    try {
      ...
      await waitForNarrationWindow(page, roadmapPlayback, 4_500);
      ...
      await setVideoCallout(page, page.getByTestId('investor-hero'), DIRECTOR_VIDEO_COPY.dashboardHeroCallout);
      await ui.pause(1_900);
      await page.getByTestId('investor-month-select').selectOption({ index: 2 });
      await expect(page.getByTestId('investor-trend-panel')).toContainText('12 tháng');
      await setVideoCallout(page, page.getByTestId('investor-metric-grid'), DIRECTOR_VIDEO_COPY.dashboardMetricsCallout);
      await ui.pause(1_900);
      await clickLocator(page, page.getByTestId('investor-export-csv'));
      await expect(page.getByTestId('investor-export-success')).toBeVisible();
      await setVideoCallout(page, page.getByTestId('investor-export-csv'), DIRECTOR_VIDEO_COPY.dashboardExportCallout);
      await ui.pause(1_600);
      await setVideoCallout(page, page.getByTestId('investor-panel-alerts'), DIRECTOR_VIDEO_COPY.dashboardAlertsCallout);
      ...
      await presentSceneCard(
        'scene-aging',
        DIRECTOR_VIDEO_COPY.agingSceneTitle,
        DIRECTOR_VIDEO_COPY.agingSceneDescription,
        DIRECTOR_VIDEO_COPY.agingSceneBullets,
        DIRECTOR_VIDEO_COPY.agingSceneNarration,
      );

      await ui.goto('/app/aging-report');
      await expect(page.getByRole('heading', { name: /Aging Report|Cong no/i })).toBeVisible();
      await setVideoLabel(page, DIRECTOR_VIDEO_COPY.agingLabel);
      const agingPlayback = beginNarration(
        cues,
        recordingStartedAt,
        'aging',
        DIRECTOR_VIDEO_COPY.agingNarration,
        narrationDurationMap,
      );
      await setVideoCallout(page, page.locator('.summary-grid').first(), DIRECTOR_VIDEO_COPY.agingSummaryCallout);
      await ui.pause(1_700);
      await page.locator('.filters select').first().selectOption('31-60');
      await expect(page.locator('table.data tbody tr').first()).toContainText('PH #1');
      await setVideoCallout(page, page.locator('.privacy-note').first(), DIRECTOR_VIDEO_COPY.agingPrivacyCallout);
      await ui.pause(1_700);
      await setVideoCallout(page, page.locator('table.data').first(), DIRECTOR_VIDEO_COPY.agingTableCallout);
      await ui.pause(2_000);
      await setVideoCallout(page, null, null);
      await waitForNarrationWindow(page, agingPlayback);

      await presentSceneCard(
        'scene-loans',
        DIRECTOR_VIDEO_COPY.loansSceneTitle,
        DIRECTOR_VIDEO_COPY.loansSceneDescription,
        DIRECTOR_VIDEO_COPY.loansSceneBullets,
        DIRECTOR_VIDEO_COPY.loansSceneNarration,
      );

      await ui.goto('/app/loans');
      await expect(page.getByRole('heading', { name: /Quản lý Vốn vay|Quan ly Von vay/i })).toBeVisible();
      await setVideoLabel(page, DIRECTOR_VIDEO_COPY.loansLabel);
      const loansPlayback = beginNarration(
        cues,
        recordingStartedAt,
        'loans',
        DIRECTOR_VIDEO_COPY.loansNarration,
        narrationDurationMap,
      );
      await setVideoCallout(page, page.locator('.stats').first(), DIRECTOR_VIDEO_COPY.loansSummaryCallout);
      await ui.pause(1_700);
      const loanTabs = page.locator('.tab-bar button');
      await clickLocator(page, loanTabs.nth(1));
      const loanRow = page.locator('.data-table tbody tr').filter({ hasText: 'LOAN-001' }).first();
      await expect(loanRow).toBeVisible();
      await setVideoCallout(page, loanRow, DIRECTOR_VIDEO_COPY.loansListCallout);
      await ui.pause(1_700);
      await clickLocator(page, loanRow.getByRole('button', { name: /Lịch trả nợ|Lich tra no/i }));
      const scheduleSummary = page.locator('.schedule-summary').first();
      await expect(scheduleSummary).toBeVisible();
      await setVideoCallout(page, scheduleSummary, DIRECTOR_VIDEO_COPY.loansScheduleCallout);
      await ui.pause(1_900);
      await clickLocator(page, loanTabs.nth(3));
      const historyRow = page.locator('.data-table tbody tr').first();
      await expect(historyRow).toBeVisible();
      await setVideoCallout(page, historyRow, DIRECTOR_VIDEO_COPY.loansHistoryCallout);
      await ui.pause(2_000);
      await setVideoCallout(page, null, null);
      ...
      const overviewStats = page.locator('section.stats').first();
      ...
      await setVideoCallout(page, overviewStats, DIRECTOR_VIDEO_COPY.adsOverviewCallout);
      await ui.pause(1_700);
      ...
      const suggestionGuard = page.locator('.suggestion-section').first();
      await expect(suggestionGuard).toBeVisible();
      await setVideoCallout(page, suggestionGuard, DIRECTOR_VIDEO_COPY.adsGuardCallout);
      ...
      await presentSceneCard(
        'scene-teaching',
        DIRECTOR_VIDEO_COPY.teachingSceneTitle,
        DIRECTOR_VIDEO_COPY.teachingSceneDescription,
        DIRECTOR_VIDEO_COPY.teachingSceneBullets,
        DIRECTOR_VIDEO_COPY.teachingSceneNarration,
      );

      await ui.goto('/app/teaching-report');
      await expect(page.getByTestId('report-tab-completed')).toBeVisible();
      await setVideoLabel(page, DIRECTOR_VIDEO_COPY.teachingLabel);
      const teachingPlayback = beginNarration(
        cues,
        recordingStartedAt,
        'teaching',
        DIRECTOR_VIDEO_COPY.teachingNarration,
        narrationDurationMap,
      );
      await clickLocator(page, page.getByTestId('report-tab-completed'));
      const completedCard = page.locator('.session-card.completed').first();
      await expect(completedCard).toBeVisible();
      await setVideoCallout(page, page.locator('.filter-panel').first(), DIRECTOR_VIDEO_COPY.teachingFilterCallout);
      await ui.pause(1_700);
      await setVideoCallout(page, page.locator('.shareholder-note').first(), DIRECTOR_VIDEO_COPY.teachingShareholderCallout);
      await ui.pause(1_700);
      await setVideoCallout(page, completedCard, DIRECTOR_VIDEO_COPY.teachingCardCallout);
      await ui.pause(1_600);
      await clickLocator(page, completedCard.locator('.session-header').first());
      const reportView = completedCard.locator('.report-view').first();
      await expect(reportView).toBeVisible();
      await setVideoCallout(page, reportView, DIRECTOR_VIDEO_COPY.teachingDetailCallout);
      await ui.pause(2_000);
      await setVideoCallout(page, null, null);
      await waitForNarrationWindow(page, teachingPlayback);

      await presentSceneCard(
        'scene-comprehensive',
        DIRECTOR_VIDEO_COPY.comprehensiveSceneTitle,
        DIRECTOR_VIDEO_COPY.comprehensiveSceneDescription,
        DIRECTOR_VIDEO_COPY.comprehensiveSceneBullets,
        DIRECTOR_VIDEO_COPY.comprehensiveSceneNarration,
      );

      await ui.goto('/app/comprehensive-report');
      await expect(page.getByTestId('comprehensive-summary')).toBeVisible();
      await setVideoLabel(page, DIRECTOR_VIDEO_COPY.comprehensiveLabel);
      const comprehensivePlayback = beginNarration(
        cues,
        recordingStartedAt,
        'comprehensive',
        DIRECTOR_VIDEO_COPY.comprehensiveNarration,
        narrationDurationMap,
      );
      await setVideoCallout(page, page.getByTestId('comprehensive-summary'), DIRECTOR_VIDEO_COPY.comprehensiveSummaryCallout);
      await ui.pause(1_700);
      await page.getByTestId('comprehensive-class-filter').selectOption('class-shareholder-001');
      await setVideoCallout(page, page.locator('.privacy-banner').first(), DIRECTOR_VIDEO_COPY.comprehensivePrivacyCallout);
      await ui.pause(1_700);
      const comprehensiveTable = page.getByTestId('comprehensive-table-container').first();
      await expect(comprehensiveTable).toBeVisible();
      await setVideoCallout(page, comprehensiveTable, DIRECTOR_VIDEO_COPY.comprehensiveTableCallout);
      await ui.pause(2_100);
      await setVideoCallout(page, null, null);
      await waitForNarrationWindow(page, comprehensivePlayback);
      ...
      await ui.goto('/app/investor-dashboard');
      await expect(page.getByTestId('investor-dashboard-page')).toBeVisible();
    ...
    expect(state.analytics.captured.realized.length).toBeGreaterThan(0);
    expect(state.analytics.captured.parents.length).toBeGreaterThan(0);
  });
});

test.describe.skip('Legacy director workflow video', () => {
  test(VIDEO_TEST_TITLE, async ({ page }) => {
    test.setTimeout(900_000);
