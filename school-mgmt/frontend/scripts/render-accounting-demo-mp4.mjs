...
const inputVideoPath = path.join(videoDir, `UI_KeToan_Full_Workflow_VI_${dateStamp}.webm`);
const outputVideoPath = path.join(videoDir, `UI_KeToan_Full_Workflow_VI_${dateStamp}.mp4`);

async function latestAccountingArtifactDir() {
  const entries = await readdir(metaDir, { withFileTypes: true });
  const directories = entries
    .filter((entry) => entry.isDirectory() && entry.name.startsWith('accounting-demo-'))
    .map((entry) => path.join(metaDir, entry.name));

  if (directories.length === 0) {
    throw new Error(`[render-accounting-demo-mp4] No accounting-demo-* directory found in ${metaDir}`);
  }
...
async function main() {
  ...
  const artifactDir = await latestAccountingArtifactDir();

  await renderNarratedVideo({
    logPrefix: 'render-accounting-demo-mp4',
    inputVideoPath,
    outputVideoPath,
    artifactDir,
    voiceoverTitle: 'Kịch bản lồng tiếng Kế toán',
    tempDirPrefix: 'accounting-demo-narration-',
  });

  console.log(`[render-accounting-demo-mp4] Output: ${outputVideoPath}`);
}
