...
const inputVideoPath = path.join(videoDir, `UI_Sale_Full_Workflow_VI_${dateStamp}.webm`);
const outputVideoPath = path.join(videoDir, `UI_Sale_Full_Workflow_VI_${dateStamp}.mp4`);

async function latestSaleArtifactDir() {
  const entries = await readdir(metaDir, { withFileTypes: true });
  const directories = entries
    .filter((entry) => entry.isDirectory() && entry.name.startsWith('sale-demo-'))
    .map((entry) => path.join(metaDir, entry.name));

  if (directories.length === 0) {
    throw new Error(`[render-sale-demo-mp4] No sale-demo-* directory found in ${metaDir}`);
  }
...
async function main() {
  ...
  const artifactDir = await latestSaleArtifactDir();

  await renderNarratedVideo({
    logPrefix: 'render-sale-demo-mp4',
    inputVideoPath,
    outputVideoPath,
    artifactDir,
    voiceoverTitle: 'Kịch bản lồng tiếng Sale',
    tempDirPrefix: 'sale-demo-narration-',
  });

  console.log(`[render-sale-demo-mp4] Output: ${outputVideoPath}`);
}
