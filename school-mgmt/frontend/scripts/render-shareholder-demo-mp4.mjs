...
const inputVideoPath = path.join(videoDir, `UI_CoDong_Full_Workflow_VI_${dateStamp}.webm`);
const outputVideoPath = path.join(videoDir, `UI_CoDong_Full_Workflow_VI_${dateStamp}.mp4`);

async function latestShareholderArtifactDir() {
  const entries = await readdir(metaDir, { withFileTypes: true });
  const directories = entries
    .filter((entry) => entry.isDirectory() && entry.name.startsWith('shareholder-demo-'))
    .map((entry) => path.join(metaDir, entry.name));

  if (directories.length === 0) {
    throw new Error(`[render-shareholder-demo-mp4] No shareholder-demo-* directory found in ${metaDir}`);
  }
...
async function main() {
  ...
  const artifactDir = await latestShareholderArtifactDir();

  await renderNarratedVideo({
    logPrefix: 'render-shareholder-demo-mp4',
    inputVideoPath,
    outputVideoPath,
    artifactDir,
    voiceoverTitle: 'Kich ban long tieng Co dong',
    tempDirPrefix: 'shareholder-demo-narration-',
  });

  console.log(`[render-shareholder-demo-mp4] Output: ${outputVideoPath}`);
}
