...
const inputVideoPath = path.join(videoDir, `UI_Director_Full_Workflow_VI_${dateStamp}.webm`);
const outputVideoPath = path.join(videoDir, `UI_Director_Full_Workflow_VI_${dateStamp}.mp4`);

async function latestDirectorArtifactDir() {
  const entries = await readdir(metaDir, { withFileTypes: true });
  const directories = entries
    .filter((entry) => entry.isDirectory() && entry.name.startsWith('director-demo-'))
    .map((entry) => path.join(metaDir, entry.name));

  if (directories.length === 0) {
    throw new Error(`[render-director-demo-mp4] No director-demo-* directory found in ${metaDir}`);
  }
...
async function main() {
  ...
  const artifactDir = await latestDirectorArtifactDir();

  await renderNarratedVideo({
    logPrefix: 'render-director-demo-mp4',
    inputVideoPath,
    outputVideoPath,
    artifactDir,
    voiceoverTitle: 'Kịch bản lồng tiếng Director',
    tempDirPrefix: 'director-demo-narration-',
  });

  console.log(`[render-director-demo-mp4] Output: ${outputVideoPath}`);
}
