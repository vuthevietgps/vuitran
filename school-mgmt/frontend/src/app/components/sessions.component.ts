        ...
        <tr *ngFor="let s of sessions()" [class]="'row-' + s.status.toLowerCase()">
          ...
          <td class="actions-cell">
            ...
            <button
              ...
              *ngIf="s.status === 'TEACHER_COMPLETED' && isParent()">
              Xác nhận
            </button>

            <button
              class="ghost sm"
              (click)="finalizeSession(s)"
              *ngIf="s.status === 'TEACHER_COMPLETED' && canCreate()">
              Chốt
            </button>
    ...
    <div class="modal-backdrop" *ngIf="showDetailModal()">
      <div class="modal modal-lg">
        ...
        <div class="detail-grid" *ngIf="selectedSession()">
          ...
          <div class="detail-row">
            <span>Lương GV:</span>
            <span>{{ formatCurrency(selectedSession()?.teacherPayout || 0) }}</span>
          </div>
          <div class="detail-row">
...
export class SessionsComponent implements OnInit {
  ...
  viewDetail(session: SessionItem): void {
