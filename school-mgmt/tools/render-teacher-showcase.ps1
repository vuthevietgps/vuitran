...
$videoPath = Join-Path $projectRoot ("..\frontend-ui-evidence\{0}\videos\UI_GiaoVien_Full_Workflow_VI_{1}.mp4" -f $RunDate, ($RunDate -replace '-', ''))
$frontendLogPath = Join-Path $runtimeLogDir ("teacher-demo-frontend-{0}.log" -f ($RunDate -replace '-', ''))
...
if (Get-NetTCPConnection -LocalPort $Port -State Listen -ErrorAction SilentlyContinue) {
  throw "Port $Port is already in use. Choose another port before running render-teacher-showcase.ps1."
}
...
try {
  ...
  & $playwrightBin test -c playwright.config.ts e2e/teacher-master-workflow.spec.ts --grep "Teacher full workflow video"
  if ($LASTEXITCODE -ne 0) {
    throw "Playwright teacher demo recording failed with exit code $LASTEXITCODE."
  }

  & node scripts/render-teacher-demo-mp4.mjs --run-date $RunDate
  if ($LASTEXITCODE -ne 0) {
    throw "MP4 render failed with exit code $LASTEXITCODE."
  }

  Write-Host "Teacher showcase video ready: $videoPath"
}
finally {
  if ($frontendPushed) {
    Pop-Location
