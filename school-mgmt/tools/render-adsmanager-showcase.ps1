...
$videoPath = Join-Path $projectRoot ("..\frontend-ui-evidence\{0}\videos\UI_AdsManager_Full_Workflow_VI_{1}.mp4" -f $RunDate, ($RunDate -replace '-', ''))
$frontendLogPath = Join-Path $runtimeLogDir ("adsmanager-demo-frontend-{0}.log" -f ($RunDate -replace '-', ''))
...
if (Get-NetTCPConnection -LocalPort $Port -State Listen -ErrorAction SilentlyContinue) {
  throw "Port $Port is already in use. Choose another port before running render-adsmanager-showcase.ps1."
}
...
try {
  ...
  & $playwrightBin test -c playwright.config.ts e2e/adsmanager-master-workflow.spec.ts --grep "Ads manager full workflow video"
  if ($LASTEXITCODE -ne 0) {
    throw "Playwright ads manager demo recording failed with exit code $LASTEXITCODE."
  }

  & node scripts/render-adsmanager-demo-mp4.mjs --run-date $RunDate
  if ($LASTEXITCODE -ne 0) {
    throw "MP4 render failed with exit code $LASTEXITCODE."
  }

  Write-Host "Ads Manager showcase video ready: $videoPath"
}
finally {
  if ($frontendPushed) {
    Pop-Location
