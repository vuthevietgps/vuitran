...
$videoPath = Join-Path $projectRoot ("..\frontend-ui-evidence\{0}\videos\UI_Director_Full_Workflow_VI_{1}.mp4" -f $RunDate, ($RunDate -replace '-', ''))
$frontendLogPath = Join-Path $runtimeLogDir ("director-demo-frontend-{0}.log" -f ($RunDate -replace '-', ''))
...
if (Get-NetTCPConnection -LocalPort $Port -State Listen -ErrorAction SilentlyContinue) {
  throw "Port $Port is already in use. Choose another port before running render-director-showcase.ps1."
}
...
try {
  ...
  & $playwrightBin test -c playwright.config.ts e2e/director-master-workflow.spec.ts --grep "Director full workflow video"
  if ($LASTEXITCODE -ne 0) {
    throw "Playwright director demo recording failed with exit code $LASTEXITCODE."
  }

  & node scripts/render-director-demo-mp4.mjs --run-date $RunDate
  if ($LASTEXITCODE -ne 0) {
    throw "MP4 render failed with exit code $LASTEXITCODE."
  }

  Write-Host "Director showcase video ready: $videoPath"
}
finally {
  if ($frontendPushed) {
    Pop-Location
